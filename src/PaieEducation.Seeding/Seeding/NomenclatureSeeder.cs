...
public sealed class NomenclatureSeeder
{
    ...
    // ----- TypesSexe (Lot 1.3) --------------------------------------------------
    private static readonly (string Id, string Libelle)[] _typesSexe =
    {
        ("M", "Masculin"),
        ("F", "Féminin"),
    };

    // ----- Situations familiales (Lot 1.3) -------------------------------------
    private static readonly (string Id, string Libelle)[] _situationsFamiliales =
    {
        ...
        CancellationToken ct = default)
    {
        ...
        await InsertTypesSexeAsync(conn, report, ct).ConfigureAwait(false);
        await InsertSituationsFamilialesAsync(conn, report, ct).ConfigureAwait(false);
    ...
    // -------------------------------------------------------------------------
    // TypesSexe (codé en dur, Lot 1.3)
    // -------------------------------------------------------------------------
    private static async Task InsertTypesSexeAsync(
        SqliteConnection c, SeedReport r, CancellationToken ct)
    {
        var inserted = 0;
        using var tx = c.BeginTransaction();
        foreach (var (id, libelle) in _typesSexe)
        {
            ct.ThrowIfCancellationRequested();
            var sql = """
                INSERT INTO TypesSexe (Id, Libelle, Actif, CreatedAt, Source, Hash)
                VALUES ($id, $l, 1, $at, 'Lot 1.3 — externalisation validations', $h)
                ON CONFLICT(Id) DO NOTHING;
                """;
            var n = await c.ExecuteAsync(sql, new
            {
        ...
        tx.Commit();
        r.Add("TypesSexe", _typesSexe.Length, inserted);
    }

    // -------------------------------------------------------------------------
    // SituationsFamiliales (codé en dur, Lot 1.3)
    // -------------------------------------------------------------------------
    private static async Task InsertSituationsFamilialesAsync(
        SqliteConnection c, SeedReport r, CancellationToken ct)
    {
        var inserted = 0;
        using var tx = c.BeginTransaction();
        foreach (var (id, libelle) in _situationsFamiliales)
        {
            ct.ThrowIfCancellationRequested();
            var sql = """
                INSERT INTO SituationsFamiliales (Id, Libelle, Actif, CreatedAt, Source, Hash)
                VALUES ($id, $l, 1, $at, 'Lot 1.3 — externalisation validations', $h)
                ON CONFLICT(Id) DO NOTHING;
                """;
            var n = await c.ExecuteAsync(sql, new
        ...
        tx.Commit();
        r.Add("SituationsFamiliales", _situationsFamiliales.Length, inserted);
    }
