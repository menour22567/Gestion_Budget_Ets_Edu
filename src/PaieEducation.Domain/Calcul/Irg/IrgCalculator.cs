...
/// exonération ≤ 30 000 → lissage spécial (handicapé/retraité RG) ou général.
/// </summary>
/// <remarks>
...
public sealed class IrgCalculator
{
    ...
    public Result<IrgResultat> Calculer(decimal revenuImposable, ProfilFiscal profil, IrgReglePeriode regle)
    {
        ...
        // 4. Exonération : si activée (seuil > 0) et revenu ≤ 30 000 → IRG = 0.
        if (regle.ExonerationSeuil > 0 && revenuImposable <= SeuilExoneration)
            return Ok(revenuImposable, brut, abattement, 0m, "exoneration");
        ...
        if (profil == ProfilFiscal.HandicapeOuRetraiteRG
            ...
            && revenuImposable > SeuilExoneration
            && revenuImposable < regle.PlafondSpecial)
        {
            ...
            return Ok(revenuImposable, brut, abattement, Math.Max(0m, lisse), "lissage_special");
        }

        // 6. Lissage général (30 000 < revenu < 35 000).
        if (EstActif(regle.CoefGeneral, regle.ConstGeneral)
            && revenuImposable > SeuilExoneration
            && revenuImposable < PlafondLissageGeneral)
        {
            var lisse = regle.CoefGeneral.Multiplier(apresAbattement) - regle.ConstGeneral.VersDecimal();
            return Ok(revenuImposable, brut, abattement, Math.Max(0m, lisse), "lissage_general");
        }

        // 7. Sinon : IRG après abattement (planché à 0).
        return Ok(revenuImposable, brut, abattement, apresAbattement, "standard");
    }
