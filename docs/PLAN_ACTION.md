...
### Précisions post-livraison 22/07/2026 (P22, P23, P11, P10)
...
| [x] **P11** (export PDF rapport d'impact) | Export PDF horodaté et archivable de tout `RapportImpact` produit par `SimulerEvolutionReglementaire`, avec référence du chemin dans `AuditLog.Payload` (D8 — auditabilité des évolutions). É4 clos (description d'hypothèse + horodatage + liste d'erreurs portés par l'enveloppe `RapportImpactDocument`). Livré commit `db5cd92` (22/07/2026), 9/9 tests P11 verts. |
| [x] **P10** (FormulaEditor avancé) | L'éditeur de rubrique bénéficie désormais de : (1) **validation live** déclenchée à chaque modification de `FormuleExpression` (partial `OnFormuleExpressionChanged` + helpers `FormulaNodeWalker.Compter/CollecterIdentificateurs/CollecterAppels`) avec bordure verte/rouge et compteur de nœuds ; (2) **auto-complétion** contextuelle (catalogue statique 6 fonctions + 7 variables + 8 sources + rubriques actives en base, port `IFormuleCompletionProvider`, popup WPF, insertion « intelligente » qui remplace le préfixe en cours) ; (3) **simulation agent témoin** via le use case `SimulerBulletinPourFormule` (override de la formule + recalcul pipeline + delta net vs baseline, 0 modification de base). Livré sur 4 commits `0deb1a6` / `786f7dd` / `a6214c6` / `4f73e8b` (22/07/2026), 13 nouveaux tests (5 VM + 9 provider dont 4 d'intégration), 765/765 verts. |
